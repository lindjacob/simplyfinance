...
function Chart({
  ...
  const generateChartData = () => {
    ...
    for (let i = 0; i <= years; i++) {
      ...
      if (budgetLabel === "investment") {
        yearData.investment = calculateFutureValue(
          0,
          interestRate,
          12,
          i,
          budgets.investment * income
        );
        yearData.prev = calculateFutureValue(
          0,
          interestRate,
          12,
          i,
          prevBudgets.investment * income
        );
      } else if (budgetLabel === "savings") {
  ...
  }, [budgets, income, budgetLabel, years]);
  ...
  function CustomToolTip({
    ...
    if (active && payload && payload.length) {
      const diff =
        (Number(payload[0].value?.toFixed(0)) ?? 0) -
        (payload[0].payload?.prev?.toFixed(0) ?? 0);
        ...
        <div className="container">
          ...
          {editing && (
            <p>
              Previous: $
              {new Intl.NumberFormat().format(
                payload[0].payload.prev.toFixed(0)
              )}
            </p>
          )}
          {editing && (
            <p className={diff < 0 ? "text-red-600" : "text-green-600"}>
              Difference: $
              {new Intl.NumberFormat().format(Number(diff.toFixed(0)))}
            </p>
          )}
        </div>
      );
    ...
    <div className="container">
      <div className="headerElement">
        ...
        <div className="flex justify-end w-1/2">
          <div
            className={`btn-sq ${years === 10 ? "btn-focus" : ""}`}
            onClick={() => setYears(10)}
          >
            10y
          </div>
          <div
            className={`btn-sq ${years === 20 ? "btn-focus" : ""}`}
            onClick={() => setYears(20)}
          >
            20y
          </div>
          <div
            className={`btn-sq ${years === 30 ? "btn-focus" : ""}`}
            onClick={() => setYears(30)}
          >
            30y
          </div>
          <div
            className={`btn-sq ${years === 50 ? "btn-focus" : ""}`}
            onClick={() => setYears(50)}
          >
            50y
          </div>
      ...
      <ResponsiveContainer height={400}>
        <AreaChart data={chartData}>
          ...
          {editing &&
            budgets[budgetLabel as BudgetKey] !=
              prevBudgets[budgetLabel as BudgetKey] && (
              <Area dataKey="prev" stroke="#71717a" fill="url(#colorPrev)" />
            )}
          <XAxis
            dataKey="date"
            axisLine={false}
